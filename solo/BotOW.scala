...
class GameEvaluationOW(game : Game) extends GameEvaluation(game, OW) {
    def eval(a : Action) : List[Evaluation] = {
        ...
        a match {
            ...
            case SummonAction(_, uc, r) if r.enemyGate && r.allies.none =>
                ...
                r.foes.goos.none && uc.cost == 2 && r.owner.strength(game, r.owner.at(r), self) < 2 && have(MillionFavoredOnes) |=> 900 -> "upgradable"
            ...
            case SummonAction(_, Mutant, r) =>
                ...
                r.controllers.num == 1 && r.foes.monsters.any && r.foes.goos.none |=> 60 -> "prevent losing gate"
                r.controllers.num == 1 && r.controllers.%(_.capturable).any && r.foes.goos.none |=> 600 -> "prevent losing gate"
                r.controllers.num == 1 && r.allies.num == 1 && r.foes.goos.none && r.foes.monsters.active.any |=> 4200 -> "prevent losing gate"
                r.allies.goos.num == 2 && r.foes.goos.any |=> 9000 -> "summon for defense"
                r.ownGate && r.allies.monsters.none |=> 100 -> "protect gate"
                r.controllers.num == 1 |=> 10 -> "one controller"

            case SummonAction(_, Abomination, r) =>
                true |=> -1000 -> "lets dont"

                r.allies.cultists.num == 6 && r.allies.monsters.none |=> 2000 -> "move gate away"
            ...
            case SummonAction(_, SpawnOW, r) =>
                ...
                r.controllers.num == 1 |=> 10 -> "one controller"
                r.ownGate && r.allies.monsters.none |=> 100 -> "no monsters"
            ...
            case DreadCurseRetreatToAction(_, r, l, e, uc, d) =>
                ...
                u.cultist && d.allies.monsters.any |=> 400 -> "cultist to monster capture"
                u.cultist && d.allies.goos.any |=> 500 -> "cultist to goo capture"
                e.at(d).monsters.any |=> -600 -> "cultist not to frieds"
        ...
        if (game.battle != null) {
            ...
            def elim(battle : Battle, u : UnitFigure) {
                ...
                u.is(SpawnOW) && opponent.has(Harbinger) && opponent.rolls.count(Pain) + opponent.rolls.count(Kill) >= battle.side(self).units.num && opponent.faction.power > 1 |=> 10000 -> "sacrifice sp"
            ...
            if (game.battle.attacker != self && game.battle.defender != self) {
                ...
                a match {
                    ...
                    case MillionFavoredOnesAction(_, r, uc, nw) =>
                        ...
                        uc == Acolyte && r.allies.monsters.none && r.allies.cultists.num > 1 |=> 1000 -> "diversify"
